...
function assignSafe(target, source, exclude) {
	for(const key in source) {
		if(key[key.length - 1] != '$' && !(key in exclude)) {
...
exports.argprops = function argprops( defaults, args, fixed, omits){
  omits = _.isString(omits) ? omits.split(/\s*,\s*/) : omits;
	const exclude = excludeList(omits);
	const result = {};
	assignSafe(result, defaults, excludeList);
	assignSafe(result, args, excludeList);
	assignSafe(result, fixed, excludeList);
